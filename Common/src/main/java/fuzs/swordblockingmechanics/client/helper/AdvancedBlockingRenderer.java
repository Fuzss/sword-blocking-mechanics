...
/**
 * Those transformations are directly ported from Minecraft 1.7, resulting in pixel-perfect recreation of third-person
 * sword blocking.
 * <p>
 * A lot has changed since then (the whole model system has been rewritten twice in 1.8 and 1.9, and had major changes
 * in 1.14 and 1.15), so we reset everything vanilla does now and apply every single step that was done in 1.7.
 * <p>
 * There were multiple classes and layers involved in 1.7, it is noted down below which class every transformation came
 * from.
 * <p>
...
public class AdvancedBlockingRenderer {
    ...
    public static <S extends EntityRenderState> void submitBlockingWithSword(S renderState, ArmedModel<S> model, ItemStackRenderState itemStackRenderState, HumanoidArm humanoidArm, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int packedLight) {
        ...
        applyTransformInverse(itemStackRenderState.firstLayer().itemTransform, leftHand, poseStack);
    ...
    private static void applyItemBlockingTransform(PoseStack poseStack, boolean leftHand) {
        ...
        // The old item layer transformations.
        ...
        // The old item renderer transformations.
    ...
    /**
     * This does the exact inverse of {@link ItemTransform#apply(boolean, PoseStack.Pose)} which should be applied right
     * after, so that in the end nothing has changed.
     */
